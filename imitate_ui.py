...
rootpath= 'C:/Users/dengl/activity/imitation_ui2/'
...
def draw_bg(screen,namelist,rankboard=True):#绘制背景
    ...
    txt = fontch.render("输入图片名称", True, (0,0,0))
    screen.blit(txt, (5,22))
    fontch = pygame.font.SysFont('simHei',22)
    txt = fontch.render("输入拍照次数", True, (0,0,0))
    screen.blit(txt, (370,22))
    fontch = pygame.font.SysFont('simHei',22)
    ...
    fontch = pygame.font.SysFont('simHei',22)
    txt = fontch.render("点击拍照", True, (0,0,0))
...
def print_score(screen,score,name_score):#打印
    fontch = pygame.font.SysFont('simHei',20)
    ...
    txt = fontch.render('总得分:  {0:.2f}'.format(score), True, (0,0,0))
...
def insert_picture(screen,name,horizontal=700,vertical=660):#插入图片
    path=rootpath+'\\origin_pics\\'+name
    pic = pygame.image.load(path).convert()
    size=pic.get_size()
